...
class McpDriverManager(private val presenter: MidiControllerContract.Presenter) {
    ...
    //    private val fxController: McpDriver = McpDriver("BCM8", "BCM8", "BCM10", "BCM9")
    private val mixerMcpDriver: McpDriver = McpDriver("BCM8", "BCM8", "BCM10", "BCM9")
    private val mixerMidiPots: MutableList<MidiPot> = mutableListOf()

    fun start() {
        mixerMcpDriver.start()
        (0..5).mapTo(mixerMidiPots) {
